...
const Orb = ({
  ...
  useEffect(() => {
    if (!isReady || !containerRef.current || orbImages.length === 0) return;
    ...
    const productCount = orbImages.length;
    const loadedAssets = [];
    let loadedCount = 0;

    orbImages.forEach((imageUrl) => {
      textureLoader.load(
        imageUrl,
        (texture) => {
          if (!mounted) {
          ...
          loadedAssets.push({ geometry, material, texture });
          loadedCount++;
    ...
    const createMeshes = () => {
      if (!mounted || loadedAssets.length === 0) return;

      for (let i = 0; i < totalItems; i++) {
        const phi = Math.acos(-1 + (2 * i) / totalItems);
        const theta = Math.sqrt(totalItems * Math.PI) * phi;

        const asset = loadedAssets[Math.floor(Math.random() * loadedAssets.length)];
    ...
    return () => {
      ...
      loadedAssets.forEach(({ geometry, material, texture }) => {
        geometry.dispose();
        material.dispose();
        texture.dispose();
      });
